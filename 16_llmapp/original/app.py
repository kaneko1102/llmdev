...
def index():
    ...
    # system_message を取得（未入力なら空文字）
    input_system_message = request.form.get("system_message", "").strip()

    # system_message の管理ロジック
    if input_system_message:
        # 新しく入力されたら session に保存
        session["system_message"] = input_system_message
    else:
        # 未入力なら前回のものを使う（なければ空文字）
        input_system_message = session.get("system_message", "")
        
    # ボットのレスポンスを取得（メモリに保持）
    get_bot_response(user_message, input_system_message, memory, session['thread_id'])
