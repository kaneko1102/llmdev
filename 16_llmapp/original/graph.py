...
# ===== グラフを実行する関数 =====
def stream_graph_updates(graph: StateGraph, user_message: str, system_message: str, thread_id):
    """
    ユーザーからのメッセージを元に、グラフを実行し、チャットボットの応答をストリーミングします。
    """
    response = graph.invoke(
        {"messages": [("user", user_message), ("system", system_message)]},
...
# ===== 応答を返す関数 =====
def get_bot_response(user_message, system_message, memory, thread_id):
    """
    ...
    # グラフを実行してボットの応答を取得
    return stream_graph_updates(graph, user_message, system_message, thread_id)
